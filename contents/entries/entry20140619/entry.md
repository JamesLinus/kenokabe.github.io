...
{
  return n; //自然数のn項目の数は、nであると定義する自然数を定義する関数
};
...
`naturalF`はイタレータのインデックス`n`をそのまま返す関数ですが、この意味はすなわち

**自然数のn項目の数とは `n` である**

と **自然数** を定義する関数を宣言しているのです。

このように自然数定義関数 `naturalF` をイタレータ`_.generate`により順次適用させることで、自然数の全体である無限数列 `Natural` が定義されます。
...
これが **黄金比** ですね。
黄金比は自己相似で自己の比率において対象であり、比率が不変です。
...
遅延評価戦略（CallByNeed）の実行環境の実装として本来兼ね備えてる特徴として、

- 一度評価したものは記憶しておく

ので、本来ならば、
数学の定義そのままに、計算量の最適化目的で、メモ化などのアルゴリズムテクニックに煩わされることなく、そのまま宣言的にコードできます。
